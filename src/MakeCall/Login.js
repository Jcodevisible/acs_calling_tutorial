...
export default class Login extends React.Component {
    constructor(props) {
        ...
        this.existingUserDetailsResponse = undefined;
    ...
    provisionNewUser = async () => {
        try {
            ...
            this.userDetailsResponse = await utils.provisionNewUser(this.displayName, this.userName);
    ...
    loginWithExistingUser = async () => {
        try {
            this.setState({ showSpinner: true, disableInitializeButton: true });
            this.existingUserDetailsResponse = await utils.getExistingUser(this.userName);
            this.displayName = this.existingUserDetailsResponse.displayname;
            console.log("ACS ID is " + this.existingUserDetailsResponse.ACS_ID);
            this.setState({ id: this.existingUserDetailsResponse.ACS_ID });
            console.log("ACS ID is " + this.state.id);
            await this.props.onLoggedIn({ id: this.state.id, token: this.existingUserDetailsResponse.token, displayName: this.displayName });
            this.setState({ loggedIn: true });
        } catch (error) {
            console.log(error);
        } finally {
                        ...
                        <div>
                            ...
                            <div className="ms-Grid-row">
                                <div className="ms-Grid-col ms-sm12 ms-lg6 ms-xl6 ms-xxl3">
                                    <TextField className="mt-3"
                                                defaultValue={undefined}
                                                label="Optional username (if specified, you can login with your username from next time)"
                            ...
                            <div className="mt-1">
                                <PrimaryButton className="primary-button mt-3"
                                    ...
                                    disabled={this.state.disableInitializeButton}
                                    onClick={() => this.loginWithExistingUser()}>
